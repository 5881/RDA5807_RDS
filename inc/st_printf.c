...
static int stprintf_(void (*)(char), const char *format, va_list arg); //generic print
static void st_long_itoa (long, int, int, void (*)(char)); //heavily used by printf
 ...
 *--------------------------------------------------------------------------------+
*/
static int stprintf_(void (*putc)(char), const char* str,  va_list arp)
{
...
//		if (!d) break;
		...
		if (l==0) {
			if (r > 0){      //unsigned
				st_long_itoa((unsigned long)va_arg(arp, int), r, w, (putc)); //needed for 16bit int, no harm to 32bit int
			}
			else            //signed
				st_long_itoa((long)va_arg(arp, int), r, w, (putc));
		} else if (l==1){  // long =32bit
				st_long_itoa((long)va_arg(arp, long), r, w, (putc));        //no matter if signed or unsigned
		}
	}
...
static void st_long_itoa (long val, int radix, int len, void (*putc) (char))
{
	char c, sgn = 0, pad = ' ';
	char s[20];
	int  i = 0;
