...
void st7735_init(void){
	...
	//Вот эта поебен отвечает за ориентацию дисплея
	//даташит стр 54
	st7735_sendcmd(ST7735_MADCTL);
#ifdef HORIZONTAL
	st7735_senddata(MV|MX|RGB); //mv set def is 0
	//st7735_senddata(MV|MY|RGB); //mv set def is 0
#else
	st7735_senddata(0x0);
#endif
...
#ifdef HORIZONTAL
	st7735_addrset(0,2,128,132);
#else
	st7735_addrset(0,0,128,132);
#endif
	gpio_set(STPORT,DC);
	for(uint16_t i=0;i<128*132;i++){
		spi_send(SPI,BCH);
		spi_send(SPI,BCL);
		}
	while (SPI_SR(SPI) & SPI_SR_BSY);
...
void st7735_drawchar_x6(unsigned char x,unsigned char y,char chr,
								uint16_t color, uint16_t bg_color){
	unsigned char i,j;
	unsigned char CH,CL,BCH,BCL;
	unsigned char buffer[6];
...
void st7735_string_x6_at(unsigned char x,unsigned char y,
	...
	while(*chr){
		if(*chr=='\n'){y+=48;chr++;i=0;}
		st7735_drawchar_x6(x+i,y,*chr++,color,bg_color);
		i+=35;
		}
	while (SPI_SR(SPI) & SPI_SR_BSY);
	gpio_set(STPORT,STCS);  
...
void st7735_drawgimg(uint8_t x,uint8_t y, uint8_t w,uint8_t h, uint8_t *data){
	...
	for(uint32_t i=0;i<size;i++)spi_send(SPI,*data++);
	while (SPI_SR(SPI) & SPI_SR_BSY);
	gpio_set(STPORT,STCS);	
}	
